...
| `refine`, `ready` | No | Local-only shaping workflow |
...
4. **Confirm category** with AskUserQuestion (binary: Yes/Change)
   ...
   c. **If provider is not local** — Create in external system via provider instructions. Capture external ID and URL from response. On failure, fall back to Offline Fallback Protocol.
...
6. **Create item file** in `.claude/work/items/W-NNN.md` (minimal captured template, include `**Assigned**: <initials>` field). If provider is not local, add `## Provider Integration` section with external ID, URL, type, sync status, and timestamp.
...
1. **Read item file**
2. **Invoke ideate skill** with item context
3. **Suggest relevant experts** based on category (from work-system.md mapping)
4. **Guide shaping discussion**: problem statement, proposed solution, acceptance criteria
5. **Update item file** with discussion outcomes (set status to `shaping`)
...
3. **Update item file** status to `ready`
